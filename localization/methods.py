import numpy as num
import localization.geometry as gx
from scipy.optimize import minimize, fmin_cobyla
from localization.find_centroid import maxPol
...
def great_circle_dist(lon1, lat1, lon2 ,lat2):
    return gx.E.gcd(lon1, lat1, lon2, lat2)
...
def is_disjoint(cA, fg=0):
    ...
    for i in range(l):
        for j in range(i + 1, l):
            ...
            if great_circle_dist(cA[j].c.x, cA[j].c.y, cA[i].c.x, cA[i].c.y) > cA[j].r + cA[i].r:
                return True
    return False
...
def CCA(cA, mode='2D', detail=False):
    if mode == '2D':
        from localization.shapely_2D import polygonize
    elif mode == 'Earth1':
        from localization.shapely_earth1 import polygonize
    else:
